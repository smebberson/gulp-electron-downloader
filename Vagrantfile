...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.synced_folder "../focusbooster-desktop-html5", "/focusbooster-desktop-html5"
  ...
  # configuration step 3: install git
  config.vm.provision "shell", path: "vagrant/git.sh"

  # configuration step 4: install nodejs
  config.vm.provision "shell", path: "vagrant/nodejs.sh"

  # configuration step 5: setup environment variables
  config.vm.provision "shell", path: "vagrant/env.sh"

  # configuration step 6: setup the development environment
  config.vm.provision "shell", path: "vagrant/development.sh"

  # configuration step 7: clean-up!
